...
## Where a sector's hand-placed content lives, if anyone has authored any.
## Sectors without a file here keep the scattered generation.
const SECTOR_DATA_DIRECTORY := "res://data/sectors/"
const AUTHORED_LISTS: Array[StringName] = [
	&"enemy_spawns", &"scrap_caches", &"ammunition_boxes", &"weapon_crates",
]
...
# keeps roughly five sectors resident while Terrain3D supplies one continuous
# ground surface. Diagonals stream in as the player approaches.
...
func _generate_sector(sector_id: StringName, definition: Dictionary) -> void:
	...
	# The legacy GridMaps are empty in the terrain-only pass. The hook remains for
	# future landmarks, whose blockers must be read on the main thread.
	...
	var config := {
		...
		# Read here and handed over as plain arrays: the worker thread builds a
		# detached subtree and has no business touching a Resource the rest of
		# the game may be using.
		"authored": _load_authored_content(coords),
...
## Hand-placed content for a sector, as plain lists of XZ offsets from the sector
## centre. Empty lists everywhere when the sector has no file, which is what puts
## the generator back on scattering.
func _load_authored_content(coords: Vector2i) -> Dictionary:
	var authored: Dictionary = {}
	for list_name in AUTHORED_LISTS:
		authored[list_name] = [] as Array[Vector2]
	var path := "%ssector_%d_%d.tres" % [SECTOR_DATA_DIRECTORY, coords.x, coords.y]
	if not ResourceLoader.exists(path):
		return authored
	var data := load(path) as SectorData
	if data == null:
		push_error("Sector data at '%s' is not a SectorData resource." % path)
		return authored
	var limit := SECTOR_SIZE * 0.5
	for list_name in AUTHORED_LISTS:
		var positions: Array[Vector2] = authored[list_name]
		for position: Vector2 in data.get(list_name):
			# A position outside the sector would put content in the neighbour,
			# which the neighbour then knows nothing about.
			if absf(position.x) > limit or absf(position.y) > limit:
				push_error(
					"%s: %s %s lies outside the sector (+-%.0f m); ignored."
					% [path, list_name, position, limit]
				)
				continue
	...
	for child in sector.get_children():
		...
		if child.get_child_count() > 3:
			for grandchild in child.get_children():
				# Parts of an instanced pickup carry the pickup itself as owner.
				# Re-parenting them then makes Godot warn on every single node,
				# and a generated sector is never packed back into a scene, so
				# the ownership serves nothing here.
